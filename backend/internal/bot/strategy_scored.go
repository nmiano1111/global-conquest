...
// signal, computed once by computeAttackSignals and turned into a score
// by ScoredStrategy.attackFeatures (weighted linear sum) -- kept as a
// separate raw-signal step so a future consumer could score the same
// candidates a different way without recomputing them. A nil pointer
// field means that feature is omitted for this candidate (mirrors
// attackFeatures' previous conditional-append behavior) rather than
// zeroed.
