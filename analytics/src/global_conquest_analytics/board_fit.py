...
Unlike fit.py's per-candidate weights (converted back to raw feature
scale for backend/internal/bot.Weights' hand-tuned-formula-compatible
representation), backend/internal/bot.BoardValue.Score standardizes a
live afterstate encoding itself using the same mean/std computed here,
then takes a dot product directly against the fitted coefficients -- so,
unlike fit.py, coefficients are exported on the *standardized* scale
(not converted back to raw units), and the standardizer's mean/std travel
alongside them in the same JSON file.
...
def fit_board_value(episodes: list[Episode], feature_names: list[str]) -> BoardValueFit:
    ...
    unlike fit.fit_phase, coefficients are kept on the standardized
    scale, matching that division of labor.

    Same LogisticRegressionCV recipe as fit.py/gbt_fit.py's diagnostic
    baseline (l1_ratios=(0.0,) pins pure L2/ridge regularization,
    scoring="neg_log_loss" and use_legacy_attributes=False pin their
    upcoming defaults) -- deliberately unchanged from Phase 1's validated
    naive baseline, per the plan's decision to park TD(λ) rather than
    adopt a new objective for this first live-play test.
    """
